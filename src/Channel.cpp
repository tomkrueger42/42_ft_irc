...
int Channel::add_member( User* new_member, bool is_invitation, std::string key = "" )
{
    ...
    else if ((m_flags & PRIVATE_CHANNEL_FLAG || m_flags & INV_ONLY_CHANNEL_FLAG)
                && is_invitation == false)
    {
...
int Channel::kick_member( std::string& caller, std::string& nick, std::string& message )
{
    ...
    switch (remove_member(nick, message))
    {
    case ERR_NOTONCHANNEL:
        return ERR_USERNOTINCHANNEL; // conversion from ERR_NOTONCHANNEL to ERR_USERNOTINCHANNEL cause we change the perspective from one member to another
    case -1:
        return -1; // channel empty, will be destroyed
    }
...
int Channel::set_topic( std::string& caller, std::string& new_topic )
{
    ...
    m_topic = new_topic.substr(1); // skipping the double colon
    return 0;
}
